...
Item {
    ...
    function setCurrentIndex(index){
        if(index < 0 || index >= nav_list.count) return
        var footerStart = nav_list.count - layout_footer.count
        var footerIndex = index >= footerStart ? index - footerStart : -1
        var item = footerIndex >= 0 ? layout_footer.model[footerIndex] : nav_list.model[index]
        if(!(item instanceof FluPaneItem) || item.disabled) return
        if(item.onTapListener){
            item.onTapListener()
            return
        }
        // 外部页面宿主只接 tap、没有 url，同样需要更新选中态。
